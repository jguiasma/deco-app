...
function RequestNextAction({ request, appointment, quote, onContact, onAppointment, onQuote, onViewQuote }: { request: AdminQuoteRequest; appointment?: AdminAppointment; quote?: AdminQuote; onContact: () => void; onAppointment: () => void; onQuote: () => void; onViewQuote: () => void }) {
  ...
  if (appointment && appointment.status !== 'cancelled') return <aside className="bwd-next-action"><p className="bwd-eyebrow">Prochaine action · Étape 2</p><h3>Rendez-vous planifié.</h3><p>{appointment.appointment_mode === 'on_site' ? 'Visite sur place payante' : `${appointment.remote_platform === 'zoom' ? 'Zoom' : 'WhatsApp'} gratuit`} · {appointment.scheduled_at ? new Intl.DateTimeFormat('fr-TN', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(appointment.scheduled_at)) : 'date à confirmer'}.</p><button className="bwd-dark-button bwd-create-quote-button" onClick={onQuote}>Créer le devis après échange</button></aside>
  return <aside className="bwd-next-action"><p className="bwd-eyebrow">Prochaine action · Étape 2</p><h3>Choisissez la suite.</h3><p>Planifiez un échange si des dimensions ou matériaux doivent être confirmés ; sinon créez directement le devis final.</p><div><button onClick={onAppointment}>Planifier un rendez-vous</button><button className="bwd-dark-button bwd-create-quote-button" onClick={onQuote}>Créer le devis</button></div></aside>
}
