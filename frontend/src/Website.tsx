...
type PublicApiService = { id: number; title: string; pricing_rule?: { unit: 'm2' | 'meter' | 'piece'; calculation_method: 'fixed' | 'per_unit' | 'prorata' } | null }
type Estimate = { estimated_min: number; estimated_max: number; currency: string; note: string }
type ApiErrors = Record<string, string[]>
type ProjectDraft = { fields: { first_name: string; last_name: string; phone: string; email: string; city: string; address: string; service_id: string; service_type: string; quantity: string; dimensions: string; approximate_budget: string; description: string }; photos: File[]; estimate: Estimate }
...
function HomePage() {
  ...
  <TrustCarousel />
  <section className="bwd-cta bwd-reveal"><p className="bwd-eyebrow">Votre projet</p><h2>Vous avez un projet ?</h2><p>Demandez une première estimation ou prenez directement rendez-vous avec notre équipe.</p><div className="bwd-actions"><Link className="bwd-dark-button" to="/demander-un-devis">Demander une estimation de devis</Link><Link className="bwd-outline-button" to="/rendez-vous">Prendre un rendez-vous</Link></div></section>
</main> }
...
function QuotePage() {
  ...
  // L'estimation reste locale jusqu'à la confirmation du rendez-vous.
  const sending = false
  const successMessage = ''
  const quoteRequestId: number | null = null
  ...
  const submitQuote = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!estimate) { void calculateEstimate(); return }
    navigate('/rendez-vous', { state: { projectDraft: { fields, photos, estimate } satisfies ProjectDraft } })
  }
...
function AppointmentPage() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      let linkedRequestId = quoteRequestId ? Number(quoteRequestId) : null
      if (projectDraft) {
        ...
        if (!quoteResponse.ok) throw new Error(quoteResult?.errors ? Object.values(quoteResult.errors).flat().join(' ') : quoteResult?.message ?? 'Impossible d’enregistrer votre projet.')
        linkedRequestId = quoteResult.data.id
      }
      const response = await fetch(`${apiUrl}/appointments`, { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify({ quote_request_id: linkedRequestId, first_name: fields.first_name, last_name: fields.last_name, phone: fields.phone, email: fields.email || null, city: fields.mode === 'on_site' ? fields.city : null, address: fields.mode === 'on_site' ? fields.address : null, appointment_type: 'visit', appointment_mode: fields.mode, remote_platform: fields.mode === 'remote' ? fields.platform : null, scheduled_at: fields.scheduled_at, notes: fields.notes || null }) })
      const data = await response.json().catch(() => null)
      if (!response.ok) throw new Error(data?.errors ? Object.values(data.errors).flat().join(' ') : data?.message ?? 'Impossible d’envoyer la demande de rendez-vous.')
      setMessage('Votre demande de rendez-vous est envoyée. Notre équipe confirmera le créneau avec vous.')
    } catch (submitError) { setError(submitError instanceof Error ? submitError.message : 'Impossible d’envoyer la demande de rendez-vous.') } finally { setSending(false) }
  }

  return <Page variant="bwd-appointment-page" title="Demander un rendez-vous" subtitle={isFromEstimate ? 'Choisissez votre créneau après votre estimation. Notre équipe le confirmera ensuite.' : 'Choisissez votre créneau pour une première consultation avec notre équipe.'}><div className="bwd-form-layout"><div className="bwd-note"><strong>Visite sur place :</strong> payante, tarif confirmé par l’entreprise.<br /><br /><strong>WhatsApp ou Zoom :</strong> échange à distance gratuit.<br /><br />{isFromEstimate ? 'Le devis final sera préparé après le rendez-vous.' : 'Vous pouvez demander une estimation de devis avant ou après cette première consultation.'}</div><form className="bwd-form" onSubmit={submit}><div><label>Prénom *<input value={fields.first_name} onChange={(event) => setField('first_name', event.target.value)} required /></label><label>Nom *<input value={fields.last_name} onChange={(event) => setField('last_name', event.target.value)} required /></label></div><div><label>Téléphone *<input value={fields.phone} onChange={(event) => setField('phone', event.target.value)} required /></label><label>E-mail<input type="email" value={fields.email} onChange={(event) => setField('email', event.target.value)} /></label></div><div><label>Type de rendez-vous<select value={fields.mode} onChange={(event) => setField('mode', event.target.value)}><option value="on_site">Visite sur place — payante</option><option value="remote">À distance — gratuit</option></select></label>{fields.mode === 'remote' ? <label>Plateforme<select value={fields.platform} onChange={(event) => setField('platform', event.target.value)}><option value="whatsapp">WhatsApp — gratuit</option><option value="zoom">Zoom — gratuit</option></select></label> : <label>Ville *<input value={fields.city} onChange={(event) => setField('city', event.target.value)} required /></label>}</div>{fields.mode === 'on_site' && <label>Adresse de la visite *<input value={fields.address} onChange={(event) => setField('address', event.target.value)} required /></label>}<label>Date et heure souhaitées *<input type="datetime-local" value={fields.scheduled_at} onChange={(event) => setField('scheduled_at', event.target.value)} required /></label><label>Informations complémentaires<textarea rows={4} value={fields.notes} onChange={(event) => setField('notes', event.target.value)} placeholder="Vos disponibilités, détails à confirmer…" /></label>{error && <p className="bwd-form-error">{error}</p>}{message && <p className="bwd-form-success">{message}</p>}<button className="bwd-dark-button" disabled={sending}>{sending ? 'Envoi…' : 'Demander ce rendez-vous'}</button></form></div></Page>
}
...
type AdminAppointment = { id: number; quote_request_id: number | null; first_name: string; last_name: string; phone: string; email: string | null; city: string | null; address: string | null; appointment_type: string; appointment_mode: string; remote_platform: string | null; scheduled_at: string | null; status: string; notes: string | null; admin_notes?: string | null; quote_request?: { id: number; service?: { title: string } | null; quantity: string | number; unit: string | null; dimensions: string | null; description: string } | null }
...
function RequestProcessSteps({ request, appointment, quote }: { request: AdminQuoteRequest; appointment?: AdminAppointment; quote?: AdminQuote }) {
  const stages: Array<[string, string, boolean]> = [
    ['1', 'Contact client', request.status !== 'new'],
    ['2', 'Rendez-vous', Boolean(appointment)],
    ['3', 'Rendez-vous terminé', appointment?.status === 'completed'],
    ['4', 'Devis final', Boolean(quote)],
  ]
  return <ol className="bwd-process-steps">{stages.map(([number, label, complete]) => <li key={number} className={complete ? 'complete' : ''}><span>{number}</span>{label}</li>)}</ol>
}

function RequestNextAction({ request, appointment, quote, onContact, onAppointment, onManageAppointment, onQuote, onViewQuote }: { request: AdminQuoteRequest; appointment?: AdminAppointment; quote?: AdminQuote; onContact: () => void; onAppointment: () => void; onManageAppointment: () => void; onQuote: () => void; onViewQuote: () => void }) {
  void onAppointment
  if (quote) return <aside className="bwd-next-action bwd-next-action-complete"><p className="bwd-eyebrow">Prochaine action</p><h3>Le devis est déjà créé.</h3><p>{quote.quote_number} est au statut « {quote.status} ». Gérez son envoi ou sa réponse dans les devis finaux.</p><button className="bwd-dark-button" onClick={onViewQuote}>Voir le devis final</button></aside>
  if (request.status === 'new') return <aside className="bwd-next-action"><p className="bwd-eyebrow">Prochaine action · Étape 1</p><h3>Contactez le client.</h3><p>Appelez-le, écrivez-lui sur WhatsApp ou envoyez un e-mail pour confirmer son besoin et ses coordonnées.</p><button className="bwd-dark-button" onClick={onContact}>Marquer « Client contacté »</button></aside>
  if (appointment?.status === 'completed') return <aside className="bwd-next-action"><p className="bwd-eyebrow">Prochaine action · Étape 4</p><h3>Préparez le devis final.</h3><p>Le rendez-vous est terminé : les mesures, matériaux et contraintes peuvent maintenant être chiffrés.</p><button className="bwd-dark-button bwd-create-quote-button" onClick={onQuote}>Créer le devis final</button></aside>
  if (appointment && appointment.status !== 'cancelled') return <aside className="bwd-next-action"><p className="bwd-eyebrow">Prochaine action · Étape 3</p><h3>Rendez-vous à réaliser.</h3><p>{appointment.appointment_mode === 'on_site' ? 'Visite sur place payante' : `${appointment.remote_platform === 'zoom' ? 'Zoom' : 'WhatsApp'} gratuit`} · {appointment.scheduled_at ? new Intl.DateTimeFormat('fr-TN', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(appointment.scheduled_at)) : 'date à confirmer'}. Après l’échange, marquez-le « Terminé ».</p><button className="bwd-dark-button" onClick={onManageAppointment}>Gérer le rendez-vous</button></aside>
  return <aside className="bwd-next-action"><p className="bwd-eyebrow">Prochaine action · Étape 2</p><h3>Attendez la demande de rendez-vous.</h3><p>Le prospect reçoit, après son estimation, un bouton pour demander son rendez-vous. Vous pourrez ensuite confirmer le créneau reçu.</p></aside>
}

function AppointmentDetail({ appointment, date, onBack, onUpdate, busy }: { appointment: AdminAppointment; date: (value: string | null) => string; onBack: () => void; onUpdate: (status: string) => void; busy: boolean }) {
  const mode = appointment.appointment_mode === 'on_site' ? 'Visite sur place — payante' : `${appointment.remote_platform === 'zoom' ? 'Zoom' : 'WhatsApp'} — gratuit`
  return <section className="bwd-admin-panel bwd-request-detail bwd-appointment-detail"><button className="bwd-back-button" onClick={onBack}>← Tous les rendez-vous</button><div className="bwd-request-detail-head"><div><p className="bwd-eyebrow">Rendez-vous #{appointment.id}</p><h2>{appointment.first_name} {appointment.last_name}</h2><p>{mode} · {date(appointment.scheduled_at)}</p></div><select className={'bwd-status bwd-status-' + appointment.status} value={appointment.status} disabled={busy} onChange={(event) => onUpdate(event.target.value)}><option value="requested">À confirmer</option><option value="confirmed">Confirmé</option><option value="completed">Terminé</option><option value="cancelled">Annulé</option></select></div><div className="bwd-request-detail-grid"><article><h3>Coordonnées</h3><p><strong>Téléphone</strong>{appointment.phone}</p><p><strong>E-mail</strong>{appointment.email ?? 'Non renseigné'}</p><p><strong>Ville</strong>{appointment.city ?? 'Non renseignée'}</p><p><strong>Adresse</strong>{appointment.address ?? 'Non renseignée'}</p></article><article><h3>Rendez-vous</h3><p><strong>Type</strong>{mode}</p><p><strong>Date et heure</strong>{date(appointment.scheduled_at)}</p><p><strong>Statut</strong>{appointment.status}</p><p><strong>Référence projet</strong>{appointment.quote_request ? 'Demande #' + appointment.quote_request.id : 'Première consultation'}</p></article></div>{appointment.quote_request && <article className="bwd-request-description"><h3>Projet demandé</h3><p><strong>{appointment.quote_request.service?.title ?? 'Service à préciser'}</strong><br />Quantité : {appointment.quote_request.quantity} {appointment.quote_request.unit ?? ''}<br />Dimensions : {appointment.quote_request.dimensions ?? 'Non renseignées'}<br /><br />{appointment.quote_request.description}</p></article>}{appointment.notes && <article className="bwd-request-description"><h3>Notes du prospect</h3><p>{appointment.notes}</p></article>}{appointment.admin_notes && <article className="bwd-request-description"><h3>Notes internes</h3><p>{appointment.admin_notes}</p></article>}</section>
}
...
function AdminWorkspace({ token, logout }: { token: string; logout: () => void }) {
  ...
  const selectedRequestAppointment = appointments.find((appointment) => appointment.quote_request_id === selectedRequest?.id)
  const selectedRequestQuote = quotes.find((quote) => quote.quote_request_id === selectedRequest?.id)
  ...
  return <main className="bwd-admin-page"><div className="bwd-admin-shell bwd-admin-app"><aside className="bwd-admin-sidebar"><Link to="/" className="bwd-admin-logo">Ben Wada<br />Déco</Link><p>Administration</p><nav>{menu.map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={tab === key ? 'active' : ''}>{label}</button>)}</nav><button className="bwd-admin-logout" onClick={logout}>Déconnexion</button></aside><section className="bwd-admin-content"><header className="bwd-admin-header"><div><p className="bwd-eyebrow">Espace professionnel</p><h1>{menu.find(([key]) => key === tab)?.[1]}</h1></div><button type="button" className="bwd-outline-button" onClick={() => void load()} disabled={busy}>Actualiser</button></header>{error && <p className="bwd-admin-error">{error}</p>}{notice && <p className="bwd-form-success">{notice}</p>}
    ...
    {tab === 'requests' && selectedRequest && <RequestProcessSteps request={selectedRequest} appointment={selectedRequestAppointment} quote={selectedRequestQuote} />}
    {tab === 'requests' && selectedRequest && <RequestNextAction request={selectedRequest} appointment={selectedRequestAppointment} quote={selectedRequestQuote} onContact={() => { const status = 'contacted'; setSelectedRequest({ ...selectedRequest, status }); void update(`quote-requests/${selectedRequest.id}`, { status }, 'Le client est marqué comme contacté.') }} onAppointment={() => openAppointment(selectedRequest)} onManageAppointment={() => setTab('appointments')} onQuote={() => openQuote(selectedRequest)} onViewQuote={() => setTab('quotes')} />}
    {tab === 'appointments' && (selectedAppointment ? <AppointmentDetail appointment={selectedAppointment} date={date} busy={busy} onBack={() => setSelectedAppointment(null)} onUpdate={(status) => { setSelectedAppointment({ ...selectedAppointment, status }); void update('appointments/' + selectedAppointment.id, { status }, 'Statut du rendez-vous mis à jour.') }} /> : <section className="bwd-admin-panel"><h2>Rendez-vous</h2><p>Ouvrez un rendez-vous pour consulter toutes les informations du prospect et de son projet.</p><div className="bwd-admin-list">{appointments.map((appointment) => <article key={appointment.id}><div><strong>{appointment.first_name} {appointment.last_name}</strong><span>{appointment.appointment_mode === 'on_site' ? 'Visite sur place — payante' : (appointment.remote_platform === 'zoom' ? 'Zoom' : 'WhatsApp') + ' — gratuit'}</span><p>{date(appointment.scheduled_at)}</p><small>{appointment.phone} · {appointment.quote_request?.service?.title ?? 'Première consultation'}</small></div><div className="bwd-admin-actions"><select className={'bwd-status bwd-status-' + appointment.status} value={appointment.status} onChange={(event) => void update('appointments/' + appointment.id, { status: event.target.value }, 'Statut du rendez-vous mis à jour.')}><option value="requested">À confirmer</option><option value="confirmed">Confirmé</option><option value="completed">Terminé</option><option value="cancelled">Annulé</option></select><button onClick={() => setSelectedAppointment(appointment)}>Voir le rendez-vous</button></div></article>)}</div>{!appointments.length && <p>Aucun rendez-vous pour le moment.</p>}</section>)}
