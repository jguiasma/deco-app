...
type AdminQuoteRequest = { id: number; first_name: string; last_name: string; phone: string; email: string | null; city: string | null; service_type: string | null; quantity: string | number; unit: string | null; estimated_min: string | number | null; estimated_max: string | number | null; description: string; status: string; created_at: string | null; service?: { title: string } }
...
type AdminQuote = { id: number; quote_number: string; quote_request_id: number | null; status: string; total: string | number; currency: string; created_at: string | null }
...
function AdminWorkspace({ token, logout }: { token: string; logout: () => void }) {
  ...
  return <main className="bwd-admin-page"><div className="bwd-admin-shell bwd-admin-app"><aside className="bwd-admin-sidebar"><Link to="/" className="bwd-admin-logo">Ben Wada<br />Déco</Link><p>Administration</p><nav>{menu.map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={tab === key ? 'active' : ''}>{label}</button>)}</nav><button className="bwd-admin-logout" onClick={logout}>Déconnexion</button></aside><section className="bwd-admin-content"><header className="bwd-admin-header"><div><p className="bwd-eyebrow">Espace professionnel</p><h1>{menu.find(([key]) => key === tab)?.[1]}</h1></div><button type="button" className="bwd-outline-button" onClick={() => void load()} disabled={busy}>Actualiser</button></header>{error && <p className="bwd-admin-error">{error}</p>}{notice && <p className="bwd-form-success">{notice}</p>}
    ...
    {tab === 'requests' && <section className="bwd-admin-panel"><h2>Demandes reçues</h2><div className="bwd-admin-list">{requests.map((request) => <article key={request.id}><div><strong>{request.first_name} {request.last_name}</strong><span>{request.service?.title ?? 'Service'} · {request.quantity} {request.unit ?? ''}</span><p>{request.description}</p><small>{request.phone} · {request.email ?? 'Sans e-mail'} · {date(request.created_at)}</small></div><div className="bwd-admin-actions"><select className={`bwd-status bwd-status-${request.status}`} value={request.status} onChange={(event) => void update(`quote-requests/${request.id}`, { status: event.target.value }, 'Statut de la demande mis à jour.')}><option value="new">Nouvelle</option><option value="contacted">Client contacté</option><option value="quoted">Devis envoyé</option><option value="accepted">Acceptée</option><option value="rejected">Refusée</option></select>{request.status === 'contacted' && <button className="bwd-dark-button bwd-create-quote-button" onClick={() => openQuote(request)}>Créer le devis</button>}</div></article>)}</div>{!requests.length && <p>Aucune demande pour le moment.</p>}</section>}
