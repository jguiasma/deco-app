...
function RequestNextAction({ request, appointment, quote, onContact, onAppointment, onManageAppointment, onQuote, onViewQuote }: { request: AdminQuoteRequest; appointment?: AdminAppointment; quote?: AdminQuote; onContact: () => void; onAppointment: () => void; onManageAppointment: () => void; onQuote: () => void; onViewQuote: () => void }) {
  ...
  return <aside className="bwd-next-action"><p className="bwd-eyebrow">Prochaine action · Étape 2</p><h3>Planifiez le rendez-vous obligatoire.</h3><p>Chaque client doit passer par une visite sur place, un appel WhatsApp ou un rendez-vous Zoom avant le devis final.</p><button className="bwd-dark-button" onClick={onAppointment}>Planifier un rendez-vous</button></aside>
}
