...
function HomePage() {
  ...
  <TrustCarousel />
  <section className="bwd-cta bwd-reveal"><p className="bwd-eyebrow">Votre projet</p><h2>Vous avez un projet ?</h2><p>Commencez par une estimation indicative ; le rendez-vous permettra ensuite de préparer votre devis final.</p><div className="bwd-actions"><Link className="bwd-dark-button" to="/demander-un-devis">Demander une estimation</Link><Link className="bwd-outline-button" to="/demander-un-devis">Étudier mon projet</Link></div></section>
</main> }
...
function QuotePage() {
  ...
  const submitQuote = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setQuoteRequestId(result.data.id)
...
function AppointmentPage() {
  ...
  const [fields, setFields] = useState({ first_name: '', last_name: '', phone: '', email: '', city: '', address: '', mode: 'on_site', platform: 'whatsapp', scheduled_at: '', notes: '' })
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`${apiUrl}/appointments`, { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify({ quote_request_id: Number(quoteRequestId), first_name: fields.first_name, last_name: fields.last_name, phone: fields.phone, email: fields.email || null, city: fields.mode === 'on_site' ? fields.city : null, address: fields.mode === 'on_site' ? fields.address : null, appointment_type: 'visit', appointment_mode: fields.mode, remote_platform: fields.mode === 'remote' ? fields.platform : null, scheduled_at: fields.scheduled_at, notes: fields.notes || null }) })
      const data = await response.json().catch(() => null)
...
type AdminAppointment = { id: number; quote_request_id: number | null; first_name: string; last_name: string; phone: string; email: string | null; appointment_type: string; appointment_mode: string; remote_platform: string | null; scheduled_at: string | null; status: string; notes: string | null }
